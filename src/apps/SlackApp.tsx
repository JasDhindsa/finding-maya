...
export const SlackApp = () => {
  ...
  const slackData = state.slack[state.activeDevice] || [];

  if (slackData.length === 0) return <div className="p-4 text-white">No Slack available.</div>;

  useEffect(() => {
    if (activeChannel) {
      dispatch({ type: 'CLEAR_APP_NOTIFICATIONS', payload: { device: state.activeDevice, app: 'Slack' } });
    }
  ...
  if (activeChannel) {
    const channelData = slackData.find((c: any) => c.channel === activeChannel);
    ...
    <div className="flex flex-col h-full bg-[#1a1818] text-[#e8d8c8]">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mb-6">
          ...
          {slackData.filter((c: any) => c.channel.startsWith('#')).map((c: any) => (
            <div
              key={c.channel}
              onClick={() => setActiveChannel(c.channel)}
              className="flex items-center gap-2 px-2 py-2 rounded hover:bg-[#2a2522] cursor-pointer"
            >
              {c.channel.includes('🔒') ? <Lock size={16} className="text-[#a49484]" /> : <Hash size={16} className="text-[#a49484]" />}
              <span className={c.channel.includes('meridian') ? 'font-bold text-white' : 'text-[#d8c8b8]'}>
                {c.channel.replace('🔒', '').trim()}
              </span>
            </div>
          ))}
        </div>
        <div>
          ...
          {slackData.filter((c: any) => c.channel.startsWith('DM:')).map((c: any) => (
            <div
              key={c.channel}
              onClick={() => setActiveChannel(c.channel)}
              className="flex items-center gap-2 px-2 py-2 rounded hover:bg-[#2a2522] cursor-pointer"
            >
              ...
              <span className="font-bold text-white">{c.channel.replace('DM: ', '')}</span>
            </div>
          ))}
        </div>
      </div>
